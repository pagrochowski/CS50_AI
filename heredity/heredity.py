...
def joint_probability(people, one_gene, two_genes, have_trait):
    ...
    for person in people:
        ...
        if people[person]["mother"] is None and people[person]["father"] is None:
            # No parents, unconditional probabilities
            joint *= PROBS["gene"][genes]
        else:
            # Calculate probabilities based on parents
            ...
            # Probability of having two genes
            if genes == 2:
                joint *= (p_mother * p_father)
            # Probability of having one gene
            elif genes == 1:
                joint *= (p_mother * (1 - p_father) + (1 - p_mother) * p_father)
            # Probability of having no genes
            else:
                joint *= ((1 - p_mother) * (1 - p_father))

        # Calculate trait probability
        if person in have_trait:
            joint *= PROBS["trait"][genes][True]
        else:
            joint *= PROBS["trait"][genes][False]
